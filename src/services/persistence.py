"""Database persistence layer for tasks and notes."""
...
async def load_tasks(user_id: str) -> list[dict]:
    ...
    Returns:
        A flat list of task dicts with id, text, checked,
        parent_id, and details.
    """
    pool = get_db()
    rows = await pool.fetch(
        "SELECT id, text, checked, position, parent_id, details "
        "FROM tasks WHERE user_id = $1 ORDER BY position",
        user_id,
    )
    return [
        {
...
async def save_task(
    ...
    await pool.execute(
        "INSERT INTO tasks "
        "(id, user_id, text, checked, position, parent_id, details) "
...
async def update_task(
    ...
    query = (
        ...
        "RETURNING id, text, checked, parent_id, details"
    )
...
async def reorder_tasks(user_id: str, ids: list[str]) -> list[dict]:
    """Reorder tasks by assigning sequential positions.
...
async def create_note(user_id: str, note_id: str, title: str) -> dict:
    ...
    pool = get_db()
    await pool.execute(
        "INSERT INTO notes (id, user_id, title, content, updated_at) "
        "VALUES ($1, $2, $3, '', NOW())",
